...
//  Uni-Link
//
//  Created by Lakshya on 11/23/24.
//
    ...
    func authenticateUser() async -> Bool {
        do {
            return try await context.evaluatePolicy(
                ...
                localizedReason: "Scan your face to mark attendance"
            )
        } catch {
            return false
        }
...
// Attendance View Observable Object
class AttendanceViewModel: ObservableObject {
    ...
    func markAttendance(studentId: String) async {
        guard let location = locationManager.location else {
            alertMessage = "Location not available. Enable location services."
            showAlert = true
            return
        }
        
        if !locationManager.isWithinCampus(location: location) {
            alertMessage = "You are not within the campus."
            showAlert = true
            return
        }
...
// Location Tracker Mockup!
class LocationManager: ObservableObject {
    @Published var location: CLLocation? = CLLocation(latitude: 37.7749, longitude: -122.4194)
    
    private let campusLatitude = 37.7749
    private let campusLongitude = -122.4194
    private let campusRadius: Double = 500 // in meters
    
    func isWithinCampus(location: CLLocation) -> Bool {
        let campusLocation = CLLocation(latitude: campusLatitude, longitude: campusLongitude)
        return location.distance(from: campusLocation) <= campusRadius
    }
}
